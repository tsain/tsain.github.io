...
miro.addListener('CANVAS_CLICKED', (e) => {console.log(e)})
  ...
  miro.initialize({
    extensionPoints: {
      toolbar: {
        ...
        onClick: () => {
          miro.board.widgets.get().then(getWeekColumns);
        }
      }
    }
  });
...
function getWeekColumns(widgets){
  let weekColumns = [];
  ...
  for (let i = 0; i < weekColumns.length; i++){
    ...
    for (let j = 0; j < widgets.length; j++){
      ...
      if (isChild(wc, w)){
        ...
        weeks[wc.plainText].children.push(w);
      }
    }
  }

  updateWeeks(firstWeek, weeks);
}

function updateWeeks(firstWeek, weeks){
  let currentWeek = ISO8601_week_no(new Date());
  console.log("Current week is " + currentWeek);

  if (weeks[currentWeek] == firstWeek){
    return;
  }
  ...
  console.log("Starting at week " + start);
